...
describe("@globiguard/sdk", () => {
  ...
  it("creates an optional browser realtime client with explicit websocket auth", () => {
    ...
    const errors: string[] = [];
    const subscription = client.realtime!.subscribeQueue(orgId, {
  ...
  it("emits only one remote subscribe per channel while multiple listeners are attached", () => {
    ...
    const first = client.realtime!.subscribeQueue(orgId, {
      onEvent: vi.fn()
    });
    const second = client.realtime!.subscribeQueue(orgId, {
      onEvent: vi.fn()
    });
  ...
  it("does not double-subscribe channels added while the socket is disconnected", () => {
    ...
    const firstSubscription = client.realtime!.subscribeQueue(firstOrgId, {
      onEvent: vi.fn()
    });
    ...
    const secondSubscription = client.realtime!.subscribeQueue(secondOrgId, {
      onEvent: vi.fn()
    });
  ...
  it("validates explicit realtime auth and identifiers", () => {
    ...
    expect(() =>
      client.realtime!.subscribeWorkflow("not-a-uuid", {
        onEvent: vi.fn()
      })
    ).toThrow(GlobiguardConfigError);
  });
  ...
  it("subscribes to governed-action decision updates by authorization id", () => {
    ...
    const subscription = client.realtime!.subscribeDecision("auth_123", {
